...
namespace CustomGame
{
    public class MapSelectScene : GameScene
    {
        ...
        public MapSelectScene()
            : base()
        {
            numberOfItems = UserData.MAX_MAP_NUMBER;
        ...
        public override void Draw(SpriteBatch spriteBatch, GameTime gameTime)
        {
            ...
            spriteBatch.Draw(selectModeTexture, new Vector2(456, 168) + new Vector2(0, 43) * UserData.currentMapMode, Color.White);

            for (int i = 0; i < NUMBER_OF_ITEM_DISPLAY; i++)
            {
                spriteBatch.Draw(MapLoadManager.getMapThumbnail(startItem + i), itemPosition[i], Color.White);
                if (UserData.isMapLock(startItem + i))
                {
            ...
            if(!UserData.isMapLock(mapIndex)){                
                ...
                mapNamePos.X -= selectLevelFont.MeasureString(MapLoadManager.getMap(mapIndex).Name).X / 2;

                spriteBatch.DrawString(selectLevelFont, MapLoadManager.getMap(mapIndex).Name, mapNamePos, Color.Green);
                spriteBatch.DrawString(selectLevelFont, MapLoadManager.getMap(mapIndex).Description, new Vector2(610, 167), Color.Black, 0, Vector2.Zero,0.8f, SpriteEffects.None, 1.0f);
            }
            spriteBatch.End();
        }
