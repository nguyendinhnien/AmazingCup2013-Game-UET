...
namespace CustomGame
{
    public class ScoreScene : GameScene
    {
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            {
                spriteBatch.DrawString(scoreFont, highscore.scores[i].player_name, scorePosition[i], Color.Black);
                spriteBatch.DrawString(scoreFont, highscore.scores[i].point.ToString(), scorePosition[i]
                    + new Vector2(500, 0), Color.Black);
                //if (UserData.topScores[i] > 0)
                //{
                //    spriteBatch.DrawString(scoreFont, UserData.usernames[i], scorePosition[i], UserData.colors[i]);
                //    spriteBatch.DrawString(scoreFont, UserData.topScores[i].ToString(), scorePosition[i]
